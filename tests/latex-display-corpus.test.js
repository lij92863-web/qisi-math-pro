...
test('a bare piecewise formula keeps its \\right. delimiter inside the math island', () => {
    const source = '\\left\\{\\begin{array}{l} x+1 \\\\ y-2 \\end{array}\\right.';
    const normalized = normalizeBareLatexForDisplayText(source);

    assert.equal(normalized, `$${source}$`);
    assert.match(normalized, /\\right\.\$/, 'the empty delimiter must stay inside the math island');
    assert.doesNotMatch(normalized, /\\right\$/);
});
