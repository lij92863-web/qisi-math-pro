...
test('current shell stays below the R3 line baseline and has retired the giant owner', () => {
...
test('R3 baseline responsibility map remains a complete immutable historical snapshot', () => {
    ...
    assert.equal(new Set(rows.map(row => row.split('|')[1].trim())).size, 318);
