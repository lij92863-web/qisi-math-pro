/**
 * Reads the "Equation Native" stream out of the OLE/CFB container Word embeds for a MathType
 * equation. A DOCX stores each MathType equation as `word/embeddings/oleObjectN.bin`, which is a
 * compound file; the MTEF bytes live in a stream named "Equation Native".
 *
 * This is the container half of the deterministic DOCX formula path: it only locates bytes, it
 * never interprets them. `qisi-docx-mtef-reader.js` turns them into LaTeX.
 */
(function (root, factory) {
    ...
    const readSectorIds = (start, table, maximum) => {
        ...
        let current = start >>> 0;
        while (
            current !== END_OF_CHAIN && current !== FREE_SECTOR
            && current < 0xfffffffa && ids.length < maximum
        ) {
    ...
    const uint32Rows = bytes => {
        ...
        for (let offset = 0; offset + 4 <= bytes.length; offset += 4) {
            rows.push(view.getUint32(offset, true));
        }
        return rows;
    };
    ...
    const readFat = (bytes, header, sectorSize) => {
        ...
        if (ids.length < header.fatCount) {
            throw new Error('Extended CFB DIFAT is unsupported for this DOCX object.');
        }
    ...
    const readDirectory = (bytes, header, sectorSize, fat) => {
        ...
        for (let offset = 0; offset + 128 <= directory.length; offset += 128) {
            ...
            entries.push({
                name: decodeUtf16(row.slice(0, Math.max(0, nameLength - 2))),
                type: row[66],
                start: view.getUint32(116, true),
                size: view.getUint32(120, true) + (view.getUint32(124, true) * 0x100000000)
            });
        }
        return entries;
    };
    ...
    const parseHeader = bytes => {
        ...
        const magic = Array.from(bytes.slice(0, 8))
            ...
            .join('');
        if (magic !== 'd0cf11e0a1b11ae1' || bytes.length < 512) {
            throw new Error('Invalid CFB/OLE container.');
        }
        return {
    ...
    const extractOleStream = (value, streamName) => {
        ...
        const wanted = String(streamName).toLowerCase();
        const stream = entries.find(entry => entry.type === 2 && entry.name.toLowerCase() === wanted);
        if (!stream) return new Uint8Array();
        if (stream.size >= header.miniCutoff) {
            return readRegularStream(bytes, stream.start, stream.size, header.sectorSize, fat);
        }

        const root = entries.find(entry => entry.type === 5);
        if (!root) return new Uint8Array();
        const miniStream = readRegularStream(bytes, root.start, root.size, header.sectorSize, fat);
        const miniFatBytes = readRegularStream(
            bytes, header.firstMiniFat, header.miniFatCount * header.sectorSize, header.sectorSize, fat
        );
        const miniFat = uint32Rows(miniFatBytes);
        const ids = readSectorIds(
            stream.start, miniFat, Math.ceil(stream.size / header.miniSectorSize) + 2
        );
        return concat(ids.map(id => miniStream.slice(
            id * header.miniSectorSize, (id + 1) * header.miniSectorSize
        ))).slice(0, stream.size);
    };

    const extractEquationNative = value => extractOleStream(value, 'Equation Native');

    // The stream starts with a small header and the MTEF block may be reached through more than one
    // documented offset, so the version byte (5) is located instead of trusting one layout.
    const extractMtefFromOle = value => {
        ...
        const candidates = [headerSize, 28, 0]
            .filter((offset, index, all) => offset < equation.length && all.indexOf(offset) === index);
