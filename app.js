        const App = {
            ...
            setup() {
                ...
                const isSourcePageImageRow = (img) => {
                ...
                const isRunningFromFileProtocol = () => {
                    return window.location.protocol === 'file:';
                };
                const getLocalServiceOpenHint = () => {
                    return '当前页面不是通过本地服务打开。请在项目目录运行 npm start，然后用浏览器打开 http://localhost:3000/main.html，不要双击 main.html。';
                };

                const checkLocalDocxConvertService = async () => {
                    const baseUrl = getLocalConvertBaseUrl();

                    if (isRunningFromFileProtocol()) {
                        console.warn('[BATCH_V2][local-service-file-protocol]', {
                            href: window.location.href,
                            hint: getLocalServiceOpenHint()
                        });
                    }
                    ...
                    try {
                        ...
                        if (!data?.ok) {
                            throw new Error(data?.error || '本地转换服务返回异常');
                        }
                        ...
                        throw new Error(
                            `本地 DOCX 转 PDF 服务不可用。` +
                            `如果当前地址是 file:///.../main.html，请不要双击 main.html。` +
                            `请在项目目录运行 npm start，然后打开 http://localhost:3000/main.html。` +
                            `原始错误：${error?.message || String(error)}`
                        );
                    }
                };

                const convertDocxRecordToPdfRecord = async (fileRecord) => {
                    await checkLocalDocxConvertService();

                    const baseUrl = getLocalConvertBaseUrl();
                    const blob = await dataUrlToBlob(fileRecord.uploadPath);
                    const form = new FormData();
                    form.append('file', blob, fileRecord.filename || 'upload.docx');

                    console.groupCollapsed('[BATCH_DEBUG][local-docx-convert-start]');
                    console.log({
                    ...
                    if (!resp.ok || !data?.ok || !data?.pdfDataUrl) {
                        console.error('[BATCH_DEBUG][local-docx-convert-backend-error]', {
                            status: resp.status,
                            data
                        });

                        const hint = Array.isArray(data?.detail?.hint)
                            ? data.detail.hint.join('；')
                            : '';

                        throw new Error(
                            `${data?.error || `DOCX 转 PDF 失败：HTTP ${resp.status}`}` +
                            `${hint ? `。提示：${hint}` : ''}`
                        );
                    }

                    console.groupCollapsed('[BATCH_DEBUG][local-docx-convert-success]');
                    console.log({
                    ...
                    const pdfFilename =
                        String(fileRecord.filename || 'upload.docx')
                            .replace(/\.docx$/i, '.pdf');

                    console.log('[BATCH_DEBUG][docx-converted-pdf-record-summary]', {
                        originalFilename: fileRecord.filename,
                        originalPageRange: fileRecord.pageRange || '',
                        pdfFilename,
                        inheritedPageRange: fileRecord.pageRange || '',
                        convertedFromDocx: true
                    });

                    return {
                        ...fileRecord,
                        id: `${fileRecord.id}_converted_pdf_${Date.now()}`,
                        filename: pdfFilename,
                        serverPdfFilename: data.pdfFilename || '',
                        fileType: 'pdf',
                        uploadPath: data.pdfDataUrl,
                        role: fileRecord.role,
                        roles: getBatchFileRoles(fileRecord),
                ...
                const withTimeout = (promise, ms, label = '任务') => {
                    ...
                    const timeoutPromise = new Promise((_, reject) => {
                        timer = setTimeout(() => {
                            reject(new Error(`${label} 超时：超过 ${Math.round(ms / 1000)} 秒未返回`));
                        }, ms);
                    });
...
`;
                ...
                const isLikelyBlockImageRef = (ref = {}) => {
                    ...
                    const byteSize = Number(ref.byteSize || 0);
                    return /\.(?:png|jpe?g|gif|bmp|webp|svg|tiff?)$/.test(filename) && byteSize >= 12000;
                };
                ...
                const normalizeQuestionType = (rawType, stem = '', options = [], answer = '', fallback = '') => {
                    ...
                    return fallback || batchDefaultMeta.defaultType || '解答题';
                };
                ...
                const shouldMatchByOrder = (items, answerItems, solutionItems) => {
                    const questionKeys = (items || []).map(item => normalizeQuestionKey(item.question));
                    const supportItems = [...(answerItems || []), ...(solutionItems || [])];
                    const supportKeys = supportItems.map(item => normalizeQuestionKey(item.question));

                    const validQuestionKeys = questionKeys.filter(Boolean);
                    const validSupportKeys = supportKeys.filter(Boolean);

                    if (validQuestionKeys.length && validSupportKeys.length) return false;

                    const sameCount = items.length > 0 && supportItems.length > 0 && items.length === supportItems.length;
                ...
                const withMatchKeys = (items, matchByOrder) => (items || []).map((item, idx) => ({
                    ...item,
                    matchKey: matchByOrder ? `#${idx + 1}` : (normalizeQuestionKey(item.question) || `#${idx + 1}`)
                }));

                const mapItemsByMatchKey = (items, matchByOrder, valueKey) => {
                    const map = new Map();
                ...
                const alignSupportItemsSafely = (questions, supports, valueKey) => {
                    if (!Array.isArray(questions) || !questions.length) return supports || [];
                    if (!Array.isArray(supports) || !supports.length) return [];

                    const qKeys = questions.map(q => normalizeQuestionKey(q.question || q.order || q.index));
                    const sKeys = supports.map(s => normalizeQuestionKey(s.question));

                    const matchedCount = sKeys.filter(k => k && qKeys.includes(k)).length;
                    const matchRate = matchedCount / Math.max(1, sKeys.length);

                    if (matchRate >= 0.5) return supports;

                    if (Math.abs(questions.length - supports.length) > 1) {
                        return supports.map(s => ({
                            ...s,
                            warnings: [
                                ...(s.warnings || []),
                                '题号匹配率低，且题目数与答案/解析数差异较大，未自动顺序对齐。'
                            ]
                        }));
                    }

                    return supports.map((s, idx) => ({
                        ...s,
                        question: normalizeQuestionKey(questions[idx]?.question || questions[idx]?.order || questions[idx]?.index || s.question),
                        confidence: Math.min(Number(s.confidence || 0.8), 0.62),
                        warnings: [
                            ...(s.warnings || []),
                            `题号匹配率低，已按顺序兜底匹配到第 ${questions[idx]?.question || idx + 1} 题，请人工复核。`
                        ]
                    }));
                };
                ...
                const preferBetterOptions = (primaryOptions, fallbackOptions) => {
                    ...
                    return fallbackCount > primaryCount ? fallback : primary;
                };
                ...
                const mergeQuestionItemsWithFallback = (primaryItems = [], fallbackItems = []) => {
                    ...
                    fallbackItems.forEach((item, idx) => {
                        const key = normalizeQuestionKey(item.question || item.questionNumber || item.order);
                        if (key && !fallbackByQuestion.has(key)) fallbackByQuestion.set(key, item);
                        if (!fallbackByQuestion.has(`#${idx}`)) fallbackByQuestion.set(`#${idx}`, item);
                    });

                    const usedFallback = new Set();
                    const mergedItems = primaryItems.map((item, idx) => {
                        const key = normalizeQuestionKey(item.question || item.questionNumber || item.order);
                        const fallback = (key && fallbackByQuestion.get(key)) || fallbackByQuestion.get(`#${idx}`);
                        if (!fallback) return item;

                        usedFallback.add(fallback);
                        ...
                        return {
                            ...
                            sourceTrace: {
                                ...
                                docxFullText:
                                    ...
                                    item.docxFullText ||
                                    fallback.sourceTrace?.docxFullText ||
                                    fallback.docxFullText ||
                                    ''
                            },
                    ...
                    fallbackItems.forEach(item => {
                        if (!usedFallback.has(item)) mergedItems.push(item);
                    });
                ...
                const forceExtractOptionsFromText = (text) => {
                    ...
                    const firstAIndex = hits.findIndex(h => h.label === 'A');
                    if (firstAIndex < 0) return null;

                    const ordered = hits.slice(firstAIndex).sort((a, b) => a.start - b.start);
                    const labels = new Set(ordered.map(h => h.label));
                    if (!labels.has('A') || !labels.has('B')) return null;
                    ...
                    const cleanOptions = sanitizeChoiceOptions(options);
                    if (cleanOptions.filter(Boolean).length < 2) return null;
                ...
                const repairDraftChoiceOptionsBeforeSave = (q) => {
                    ...
                    q.warnings = (q.warnings || []).filter(w => !String(w).includes('选择题仅识别到') && !String(w).includes('选择题选项未识别完整'));
                    q.warnings.push('已从原始文本块自动补回选择题选项，请核对。');
                    return true;
                };
                ...
                const addChoiceOptionWarning = (q) => {
                    ...
                    q.mergeWarnings = [...new Set([...(q.mergeWarnings || []), 'missing_options'])];
                    return true;
                };
                ...
                const parseDocxOptionsFromText = (text = '') => {
                    const source = normalizeDocxOptionEvidenceText(text);

                    if (!source) {
                        return { stem: '', options: ['', '', '', ''], optionCount: 0 };
                    }
                    ...
                    // 方案 2：Word 表格被抽成 “A xxx B xxx C xxx D xxx”，中间没有标点。
                    {
                        const compactRegex = /(?:^|[\n\s　])A\s+(.+?)\s+B\s+(.+?)\s+C\s+(.+?)\s+D\s+(.+?)(?=\n\s*\d{1,3}[\.．、:：]|\n\s*第\s*\d{1,3}\s*题|$)/s;
                        const match = source.match(compactRegex);
                    ...
                    // 方案 4：Word 表格被抽成“标签行 + 内容行”
                    {
                        ...
                        const isOptionLabel = (value, label) => {
                            const text = String(value || '')
                                .replace(/[Ａ-Ｄ]/g, ch => String.fromCharCode(ch.charCodeAt(0) - 65248))
                                .trim()
                                .toUpperCase();
                        ...
                        const splitLabelCells = (line) => {
                            const cells = splitCells(line);
                            if (cells.length >= 4) return cells;

                            const loose = String(line || '').trim().split(/\s+/).filter(Boolean);
                            return loose.length >= 4 ? loose : cells;
                        };

                        const splitValueCells = (line) => {
                            const cells = splitCells(line);
                            if (cells.length >= 4) return cells;
                        ...
                        // 4.1 一行 A/B/C/D，下一行四个选项内容
                        for (let i = 0; i < lines.length - 1; i += 1) {
                            const labelCells = splitLabelCells(lines[i]);
                            const valueCells = splitValueCells(lines[i + 1]);

                            const isLabelRow =
                                labelCells.length >= 4 &&
                                ['A', 'B', 'C', 'D'].every((label, idx) => isOptionLabel(labelCells[idx], label));
                        ...
                        // 4.2 连续四行 A/B/C/D，后面连续四行是选项内容
                        for (let i = 0; i < lines.length - 7; i += 1) {
                            const labelOnly = ['A', 'B', 'C', 'D'].every((label, idx) =>
                                isOptionLabel(lines[i + idx], label)
                            );
                ...
                const optionCountOf = (options) => Array.isArray(options)
                    ? options.filter(opt => {
                ...
                const findVisualItemForQuestion = (visualItems = [], textItem = null, index = 0) => {
                    if (!textItem) return null;

                    const qKey = normalizeQuestionKey(
                        textItem.question ||
                        textItem.questionNumber ||
                        textItem.order ||
                        ''
                    );

                    if (qKey) {
                        const exact = (visualItems || []).find(item =>
                            normalizeQuestionKey(item.question || item.questionNumber || item.order || '') === qKey
                        );

                        if (exact) return exact;
                    }

                    return visualItems[index] || null;
                };

                const fillOptionsFromDocxVisualOnly = (textItems = [], visualItems = []) => {
                    if (!Array.isArray(textItems) || !textItems.length) return textItems;
                    if (!Array.isArray(visualItems) || !visualItems.length) return textItems;

                    return textItems.map((item, index) => {
                        const next = item;
                        const currentCount = optionCountOf(next.options);
                        ...
                        if (visualCount >= 2) {
                            ...
                            next.warnings = [...new Set([
                                ...(next.warnings || []),
                                'DOCX 文本层未抽出选项，已从页面图视觉识别结果补回选项。'
                            ])];

                            console.log('[BATCH_DEBUG][docx-visual-options-filled]', {
                                q: next.question || next.questionNumber || next.order,
                                index,
                                visualQ: visual?.question || visual?.questionNumber || visual?.order,
                                optionA: visualOptions[0] || '',
                                optionB: visualOptions[1] || '',
                                optionC: visualOptions[2] || '',
                                optionD: visualOptions[3] || ''
                            });
                        } else {
                            console.warn('[BATCH_DEBUG][docx-visual-options-missing]', {
                                q: next.question || next.questionNumber || next.order,
                                index,
                                visualQ: visual?.question || visual?.questionNumber || visual?.order,
                ...
                const fillDocxOptionsOnly = (item) => {
                    if (!item) return false;

                    const debugOptionEvidence = true;

                    if (debugOptionEvidence) {
                        const fields = [
                            ['rawBlock', item.rawBlock],
                            ['rawText', item.rawText],
                            ['pageText', item.pageText],
                            ['sourceText', item.sourceText],
                            ['docxFullText', item.docxFullText],
                            ['sourceTrace.rawBlock', item.sourceTrace?.rawBlock],
                            ['sourceTrace.pageText', item.sourceTrace?.pageText],
                            ['sourceTrace.sourceText', item.sourceTrace?.sourceText],
                            ['sourceTrace.docxFullText', item.sourceTrace?.docxFullText],
                            ['stem', item.stem]
                        ];

                        console.groupCollapsed('[BATCH_DEBUG][docx-options-evidence]');
                        console.log('q =', item.question || item.questionNumber || item.order);
                    ...
                    for (const evidence of evidenceList) {
                        ...
                        console.groupCollapsed('[BATCH_DEBUG][docx-options-parse-attempt]');
                        console.log('q =', item.question || item.questionNumber || item.order);
                        ...
                        if (count >= 2) {
                            ...
                            console.log('[BATCH_DEBUG][docx-options-filled-only]', {
                                q: item.question || item.questionNumber || item.order,
                                count,
                                optionA: options[0] || '',
                                optionB: options[1] || '',
                                optionC: options[2] || '',
                                optionD: options[3] || '',
                                evidenceHead: evidence.slice(0, 300)
                            });
                ...
                const attachDocxTextEvidenceToItem = (item, sourceFile, fullText = '') => {
                    ...
                    next.docxFullText = next.docxFullText || text;

                    // 不要覆盖已有 rawBlock；但必须保留全文证据。
                    next.pageText = next.pageText || text;
                    next.sourceText = next.sourceText || text;
                    next.pageTextOriginal = next.pageTextOriginal || text;
                    next.sourceTextOriginal = next.sourceTextOriginal || text;

                    next.sourceTrace = {
                        ...(next.sourceTrace || {}),
                        sourceFileId: next.sourceTrace?.sourceFileId || sourceFile.id,
                        sourceFileName: next.sourceTrace?.sourceFileName || sourceFile.filename || '',
                        pageText: next.sourceTrace?.pageText || text,
                        sourceText: next.sourceTrace?.sourceText || text,
                        pageTextOriginal: next.sourceTrace?.pageTextOriginal || text,
                        sourceTextOriginal: next.sourceTrace?.sourceTextOriginal || text,
                        docxFullText: next.sourceTrace?.docxFullText || text
                    };

                    return next;
                };

                const repairDocxOptionsFromTextEvidence = (draft) => {
                    if (!draft) return draft;

                    const existing = cleanDisplayOptionsForBatchSave(draft.options || []);
                    const existingCount = existing.filter(Boolean).length;

                    const qno = draft.questionNumber || draft.question || draft.order || '';

                    const fullText =
                        draft.docxFullText ||
                        draft.sourceTrace?.docxFullText ||
                        draft.pageTextOriginal ||
                        draft.sourceTextOriginal ||
                        draft.pageText ||
                        draft.sourceText ||
                        draft.sourceTrace?.pageTextOriginal ||
                        draft.sourceTrace?.sourceTextOriginal ||
                        draft.sourceTrace?.pageText ||
                    ...
                    const sources = [
                        draft.rawBlock,
                        draft.rawText,
                        draft.recognitionRaw?.rawBlock,
                        draft.recognitionRaw?.stem,
                        draft.pageText,
                        draft.sourceText,
                        draft.pageTextOriginal,
                        draft.sourceTextOriginal,
                        draft.sourceTrace?.rawBlock,
                        draft.sourceTrace?.pageText,
                        draft.sourceTrace?.sourceText,
                        draft.sourceTrace?.pageTextOriginal,
                        draft.sourceTrace?.sourceTextOriginal,
                        currentBlock,
                        draft.stem
                    ].map(normalizeDocxOptionEvidenceText).filter(Boolean);

                    const parsedList = sources
                        .map((source, idx) => {
                            const parsed = parseDocxOptionsFromText(source);
                            return {
                                ...parsed,
                                sourceIndex: idx,
                                sourceHead: source.slice(0, 180)
                            };
                        })
                        .filter(item => item && item.optionCount > existingCount)
                        .sort((a, b) => b.optionCount - a.optionCount || b.score - a.score);

                    const best = parsedList[0];

                    console.groupCollapsed('[BATCH_DEBUG][docx-option-repair]');
                    console.log('id =', draft.id);
                    console.log('q =', qno);
                    console.log('existingCount =', existingCount);
                    console.log('fullTextLength =', String(fullText || '').length);
                    console.log('currentBlockLength =', String(currentBlock || '').length);
                    console.log('currentBlockHead =', String(currentBlock || '').slice(0, 500));
                    console.table(parsedList.map(item => ({
                    ...
                    if (repairedOptionCount >= 2 && !['单选题', '多选题'].includes(draft.type)) {
                        draft.type = '单选题';
                    }
                    ...
                    if (/^[A-D]{2,4}$/.test(normalizedAnswer)) {
                        draft.type = '多选题';
                    }
                    ...
                    console.table([{
                        ...
                        q: qno,
                        type: draft.type,
                        optionCount: draft.options.filter(Boolean).length,
                ...
                const parseChoiceQuestionsFromPageMarkdown = (markdown = '', source = {}) => {
                    const blocks = splitPageMarkdownIntoQuestionBlocks(markdown);

                    return blocks.map((item, idx) => {
                        const parsed = parseOptionsFromBlock(item.block);
                        const optionCount = parsed.options.filter(Boolean).length;
                        const inferredType = optionCount >= 2 ? '单选题' : '';
                        ...
                        return {
                            ...
                            sourcePage: source.pageNo || 0,
                            pageIndex: source.pageNo || 0,
                            sourcePageImage: source.imageUrl || '',
                            confidence: 0.62,
                            warnings: optionCount >= 2
                                ? [`本题选项由 OCR Markdown 本地解析得到，请核对。`]
                                : [`本题由 OCR Markdown 本地解析得到，但未识别完整选项。`],
                            sourceTrace: {
                                sourceFileId: source.fileId || '',
                                sourceFileName: source.fileName || '',
                                sourcePage: source.pageNo || 0,
                ...
                const mergeAiQuestionsWithLocalMarkdown = (aiItems = [], localItems = []) => {
                    ...
                    locals.forEach((item, index) => {
                        ...
                        if (key && !localByQuestion.has(key)) {
                            localByQuestion.set(key, { item, index });
                        }
                    });

                    const usedLocalIndexes = new Set();

                    const getLocalForAi = (ai, index) => {
                        const key = normalizeQuestionKey(ai?.question);
                        if (key && localByQuestion.has(key)) {
                            return localByQuestion.get(key);
                        }

                        // 题号不稳定时，按页内顺序兜底合并。
                        // 这个不是跨题全文补选项，只是在同一页 OCR Markdown 的第 index 道题和 AI 第 index 道题之间合并。
                        if (locals[index]) {
                            return { item: locals[index], index };
                        }

                        return null;
                    };

                    const merged = (aiItems || []).map((ai, index) => {
                        const localRef = getLocalForAi(ai, index);
                        ...
                        next.rawBlock = next.rawBlock || local.rawBlock || '';
                        next.rawText = next.rawText || local.rawText || '';
                        next.pageText = next.pageText || local.pageText || '';
                        next.sourceText = next.sourceText || local.sourceText || '';
                        next.sourcePageImage = next.sourcePageImage || local.sourcePageImage || '';
                        next.sourcePage = next.sourcePage || local.sourcePage || 0;
                        next.pageIndex = next.pageIndex ?? local.pageIndex ?? 0;

                        next.sourceTrace = {
                            ...(local.sourceTrace || {}),
                            ...(next.sourceTrace || {}),
                            sourceFileId: next.sourceFileId || local.sourceFileId || local.sourceTrace?.sourceFileId || '',
                            sourceFileName: next.sourceFileName || local.sourceFileName || local.sourceTrace?.sourceFileName || '',
                            sourcePage: next.sourcePage || local.sourcePage || local.sourceTrace?.sourcePage || 0,
                            pageIndex: next.pageIndex ?? local.pageIndex ?? local.sourceTrace?.pageIndex ?? 0,
                            sourcePageImage: next.sourcePageImage || local.sourcePageImage || local.sourceTrace?.sourcePageImage || '',
                            rawBlock: next.rawBlock || local.rawBlock || local.sourceTrace?.rawBlock || '',
                            pageText: next.pageText || local.pageText || local.sourceTrace?.pageText || '',
                            sourceText: next.sourceText || local.sourceText || local.sourceTrace?.sourceText || ''
                        };
                        ...
                        if (localCount > aiCount) {
                            ...
                            if (!['单选题', '多选题'].includes(next.type) && localCount >= 2) {
                                next.type = local.type || '单选题';
                            }

                            next.warnings = [
                                ...(next.warnings || []),
                                `AI 未完整返回选项，已使用 OCR Markdown 本地解析补齐 ${localCount}/4 个选项，请核对。`
                            ];
                        } else {
                            next.options = aiOptions;
                        }

                        return next;
                    });

                    // AI 漏题时，把本地 Markdown 解析出的题补进去。
                    ...
                    for (const pageNo of pages) {
                        ...
                        const items = (content.items || [])
                            .map(item => {
                                ...
                                const t = window.pdfjsLib.Util.transform(viewport.transform, item.transform || [1, 0, 0, 1, 0, 0]);
                                const x = t[4] || 0;
                                const y = t[5] || 0;
                ...
                const extractImageTokenIds = (text) => [...new Set((String(text || '').match(/\[\[IMAGE:([^\]]+)\]\]/g) || [])
                    .map(token => token.match(/\[\[IMAGE:([^\]]+)\]\]/)?.[1])
                    .filter(Boolean))];
                ...
                const getDocxImageIdFromToken = (token = '') => {
                    const m = String(token || '').match(/\[\[IMAGE:([^\]]+)\]\]/);
                    return m ? m[1] : '';
                };

                const extractImageRelationshipIdsFromXml = (xml = '') => {
                    const ids = [];
                    ...
                    String(xml || '').replace(/\br:link=["']([^"']+)["']/g, (_, rid) => {
                        if (rid) ids.push(rid);
                        return '';
                    });
                ...
                const parseTextOptionsFromDocxQuestionText = (blockText = '') => {
                    const source = normalizeDocxTextSpace(blockText || '');
                    if (!source) return ['', '', '', ''];
                ...
                const parseChoiceOptionsFromDocxQuestionXml = (
                    ...
                    const labelRegex = /<w:t[^>]*>\s*([A-D])\s*[.．、:：)）]\s*<\/w:t>/g;
                    const hits = [];
                    let match;
                    ...
                    if (hits.length < 2) return { options, images: [], hasUndisplayable: false };
                    ...
                    for (let i = 0; i < hits.length; i += 1) {
                        const hit = hits[i];
                        const optionIndex = hit.label.charCodeAt(0) - 65;
                        if (optionIndex < 0 || optionIndex > 3) continue;
                        ...
                        const rids = extractImageRelationshipIdsFromXml(segment);
                        const displayableRid = rids.find(rid =>
                            ridImageMetaMap.get(rid)?.displayable && ridImageUrlMap.has(rid)
                        );
                        const anyImageRid = displayableRid || rids.find(rid => ridImageUrlMap.has(rid));

                        if (!anyImageRid) continue;

                        const meta = ridImageMetaMap.get(anyImageRid);
                        const url = ridImageUrlMap.get(anyImageRid);

                        if (url && meta?.displayable) {
                            const id = `docx_opt_${q || 'q'}_${hit.label}_${localImages.length}_${Date.now()}`;
                            const filename = `${id}.${meta.ext || 'png'}`;
                            ...
                            imageList.push(img);
                            options[optionIndex] = `[[IMAGE:${id}]]`;
                        } else {
                            hasUndisplayable = true;
                            const ext = meta?.ext || 'unknown';
                            options[optionIndex] = `[公式图片选项暂不支持预览：${ext}]`;

                            console.warn('[BATCH_DEBUG][docx-option-image-undisplayable]', {
                                q,
                                label: hit.label,
                                rid: anyImageRid,
                                target: meta?.target,
                                ...
                                mime: meta?.mime,
                                hasUrl: Boolean(url),
                                displayable: Boolean(meta?.displayable)
                            });
                        }
                    }
                ...
                const buildDocxOptionMapFromDocumentXml = (
                    documentXml = '',
                    ridImageUrlMap = new Map(),
                    ridImageMetaMap = new Map()
                ) => {
                    ...
                    for (const p of paragraphs) {
                        const line = p.text || '';
                        const qNo = getQuestionNoFromDocxLine(line);
                    ...
                    for (const block of blocks) {
                        ...
                        const blockRawXml = block.rawXml.join('\n');
                        const imageList = [];
                        let parsed = parseChoiceOptionsFromDocxQuestionXml(
                            blockRawXml,
                            ridImageUrlMap,
                            ridImageMetaMap,
                            imageList,
                        ...
                        let hasUndisplayable = Boolean(parsed.hasUndisplayable);

                        if (optionCountOf(options) < 2) {
                            options = parseTextOptionsFromDocxQuestionText(blockText);
                            images = [];
                            hasUndisplayable = false;
                        }

                        const count = optionCountOf(options);

                        console.log('[BATCH_DEBUG][docx-option-map-block]', {
                            q: block.q,
                            optionCount: count,
                            optionA: options[0] || '',
                            optionB: options[1] || '',
                            ...
                            imageExts: images.map(img => img.ext || ''),
                            hasUndisplayableOption: hasUndisplayable,
                            blockHead: blockText.slice(0, 1000)
                        });
                    ...
                    console.groupCollapsed('[BATCH_DEBUG][docx-option-map-summary]');
                    console.log('paragraphCount =', paragraphs.length);
                    console.log('blockCount =', blocks.length);
                    console.log('optionMapKeys =', [...optionMap.keys()]);
                    console.groupEnd();

                    return optionMap;
                };

                const isFormulaImageRef = (ref = {}) => {
                    const filename = String(ref.filename || '').toLowerCase();
                    const byteSize = Number(ref.byteSize || 0);
                    return /\.(?:wmf|emf)$/.test(filename) && (!byteSize || byteSize < 12000);
                };
...
const pushUniqueQuestionItem = (list, item, valueKey) => {
                ...
                const mergeAnswerSolutionResults = (localResult, aiResult) => {
                    ...
                    return {
                        answers: mergeByQuestion(localResult?.answers, aiResult?.answers, 'answer'),
                        solutions: mergeByQuestion(localResult?.solutions, aiResult?.solutions, 'solution')
                    };
                };
                ...
                const parseJsonFromAiText = (text) => {
                    ...
                    if (objectStart >= 0 && objectEnd > objectStart) {
                        const obj = tryParse(body.slice(objectStart, objectEnd + 1));
                        if (obj) return obj;
                    }
                    ...
                    if (arrayStart >= 0 && arrayEnd > arrayStart) {
                        const arr = tryParse(body.slice(arrayStart, arrayEnd + 1));
                        if (arr) return arr;
                    }

                    console.warn('AI 返回不是可解析 JSON，原文前 1000 字：', raw.slice(0, 1000));
                    return null;
                };
                        ...
                        () => {
                            const invalidLatex =
                                parseStrictQuestionPayload(
                                    String.raw`{"questions":[{"questionNumber":"1","stem":"in \triangle ABC","options":{"A":"\frac{1}{2}"}}]}`
                                );

                            const legalLatex =
                                parseStrictQuestionPayload(
                                    String.raw`{"questions":[{"stem":"in \\triangle ABC"}]}`
                                );
                            ...
                            const checks = {
                                repairsInvalidLatex:
                                    ...
                                    invalidLatex.questions[0]
                                        ?.stem ===
                                        String.raw`in \triangle ABC` &&
                                    invalidLatex.questions[0]
                                        ?.options?.A ===
                                        String.raw`\frac{1}{2}`,

                                keepsLegalJsonOnOriginalPath:
                                    legalLatex.ok &&
                                    legalLatex.method ===
                                        'JSON.parse' &&
                                    legalLatex.questions[0]
                                        ?.stem ===
                                        String.raw`in \triangle ABC`,

                                keepsJsonNewlineEscape:
                                    newlineJson.ok &&
                                    newlineJson.method ===
                                        'JSON.parse' &&
                                    newlineJson.questions[0]
                                        ?.stem ===
                                        'line1\nline2',
                                ...
                                usesExtractQuestionArray:
                                    invalidLatex.questions
                                        .length === 1
                            };
                ...
                const createStrictVisionProtocolError = ({
                    payload,
                    model = '',
                    file = null,
                    pageNo = 0,
                    ...
                    const error =
                        new Error(
                            `${model || '视觉模型'} ` +
                            `返回的严格题目结果不符合 JSON 协议：` +
                            `${reason}${detail}`
                        );

                    error.code =
                        'STRICT_VISION_PROTOCOL_ERROR';
                    ...
                    for (const item of items || []) {
                        ...
                        parsed.forEach((q, idx) => {
                            result.push({
                                ...
                                solution: '',
                                type: normalizeQuestionType(q.type || item.type, q.stem, q.options, idx === 0 ? item.answer || '' : '', batchDefaultMeta.defaultType),
...
`;
                ...
                const bindRecognizedQuestionFigures = async (draft, draftImages, files, batchId) => {
                    ...
                    const sourceImageFile = files.find(file =>
                        file.id === (
                    ...
                    const sourceUrl =
                        sourceImageFile?.uploadPath ||
                        draft.sourcePageImage ||
                        draft.sourceTrace?.sourcePageImage ||
                        '';

                    if (!sourceUrl) {
                        addWarningOnce(draft, '已识别到题中图形区域，但没有可用于裁剪的来源页面图。');
                        return;
                    }
                    ...
                    for (let index = 0; index < figures.length; index += 1) {
                        ...
                        if (!isLikelyRealQuestionFigure(figure, draft.sourceBbox || draft.question_bbox || [])) {
                            continue;
                        }
                    ...
                    draft.images = mergeImageListsById(draft.images || [], boundRefs);
                    draft.hasImage = true;
                    ...
                    draft.imageReviewStatus =
                        statuses.includes('low_confidence')
                            ? 'low_confidence'
                            : (statuses.includes('need_confirm') ? 'need_confirm' : 'confirmed');

                    if (draft.imageReviewStatus !== 'confirmed') {
                        addWarningOnce(draft, '自动裁剪的题中图形需要人工确认。');
                    }
                };
...
const logBatchPdfDiag = (stage, payload = {}, level = 'log') => {
                ...
                const renderPdfFilePages = async (file, options = {}) => {
                    ...
                    let pdf;
                    try {
                        pdf = await window.pdfjsLib.getDocument({ data: new Uint8Array(buffer) }).promise;
                    } catch (error) {
                    ...
                    if (pages.length > 20) {
                        const estimated = estimateVisionCalls(pages.length, activeRecognitionMode);
...
【输出格式】
{
  ...
  "evidence": "用于恢复的原文短摘录"
                ...
                const reconcileStrictQuestionsWithDocxSkeleton = (
                    items = [],
                    skeleton = null
                ) => {
                    const questionNumbers =
                        Array.isArray(skeleton?.questionNumbers)
                            ? skeleton.questionNumbers
                                .map(number =>
                                    normalizeQuestionKey(number)
                                )
                                .filter(Boolean)
                            : [];
...
${repairInfo ? `【需要重点修复的问题】\n${repairInfo}` : ''}`;
                ...
                const recognizeStrictQuestionsFromPreparedPages = async ({
                    ...
                    const pageResults = await mapWithConcurrency(pages, 2, async (page, index) => {
                        ...
                        if (!page.url) {
                            pageRecognitionSummary.push({
                                ...
                                errorMessage: `${file.filename} 第 ${pageNo} 页没有页面图`,
                                durationMs: Math.round(performance.now() - pageStartedAt)
                            });
                            throw new Error(`${file.filename} 第 ${pageNo} 页没有页面图`);
                        }

                        if (onPageProgress) {
                            await onPageProgress(index / Math.max(1, pages.length), {
                                stage: 'initial-page-recognition',
                                pageNo
                            });
                        }

                        const result = await recognizeStrictQuestionPageWithQwen({
                            file,
                            imageUrl: page.url,
                            pageNo,
                            expectedQuestionCount: 0,
                        ...
                        for (const item of pageItems) {
                            item.recognizedImages = collectValidRecognizedFigures(item);
                        }

                        const figureLocatorTargets = pageItems.filter(item =>
                            questionHasFigureCue(item) &&
                            collectValidRecognizedFigures(item).length === 0
                        );

                        if (figureLocatorTargets.length) {
                            try {
                                const locatedFigures = await locateQuestionFiguresWithQwen({
                                    pageImage: page.url,
                                    pageNo,
                                    sourceFileName: file.filename,
                                ...
                                for (const rawFigure of locatedFigures || []) {
                                    ...
                                    const normalized = normalizeRecognizedFigureDescriptor(rawFigure);
                                    if (!normalized) continue;
                                    ...
                                    figuresByQuestion.get(questionNumber).push(normalized);
                                }
                                ...
                                for (const item of pageItems) {
                                    ...
                                    const located = figuresByQuestion.get(questionNumber) || [];
                                    const validLocated = located.filter(figure =>
                                        isLikelyRealQuestionFigure(
                                            figure,
                                            item.question_bbox || item.sourceBbox || []
                                        )
                        ...
                        console.log('[BATCH_DEBUG][strict-page-result]', {
                            ...
                            questionNumbers: pageItems.map(item => item.questionNumber || item.question || ''),
                            figureCounts: pageItems.map(item => ({
                                questionNumber: item.questionNumber || item.question || '',
                                count: collectValidRecognizedFigures(item).length
                            }))
                        });
                    ...
                    console.log('[BATCH_TIME][strict-initial-recognition]', {
                        ...
                        durationMs: Math.round(performance.now() - recognitionStart)
                    });

                    let check = validateVisualQuestionItems(items, expected);
                    ...
                    console.log('[BATCH_DEBUG][strict-targeted-repair-start]', repairTargets);

                    if (repairTargets.length) {
                        const targetNumbers = new Set(repairTargets.map(row => String(row.questionNumber)));
                        ...
                        for (const page of targetPages) {
                            ...
                            const repaired = await repairPageChoiceAndSolutionDetailsWithVision(
                                {
                                    ...file,
                                    pageIndex: pageNo,
                                    sourcePage: pageNo
                            ...
                            items = mergeStrictQuestionItemsByNumber([
                                ...items,
                                ...(repaired || []).map(item => ({
                                    ...item,
                        ...
                        check = validateVisualQuestionItems(items, expected);

                        console.groupCollapsed('[BATCH_DEBUG][strict-after-targeted-repair]');
                        console.log({
                    ...
                    items = items.map(item => {
                        const row = check.failedQuestions.find(failed =>
                            String(failed.questionNumber) === String(item.questionNumber)
                        );
                        const warnings = [
                            ...(item.warnings || []),
                            ...(row?.warnings || []),
                    ...
                    console.log('[BATCH_DEBUG][strict-summary]', {
                        ...
                        types: items.map(item => ({
                            ...
                            type: item.type,
                            validOptionCount: normalizeFourOptionsForCheck(item.options || [])
                                .filter(option => !isBadChoiceOption(option)).length
                        }))
                    });

                    if (onPageProgress) {
                        await onPageProgress(1, {
                            stage: 'done',
                            done: true
                        });
                    }
                    ...
                    return {
                        ...
                        pageRecognitionSummary:
                            [...pageRecognitionSummary]
                                .sort(
                                    (left, right) =>
                                        Number(left.pageNo || 0) -
                                        Number(right.pageNo || 0)
                                )
                    };
                };
                ...
                const buildDocxQuestionFailureSnapshot = ({
                    ...
                    const rawQuestions =
                        Array.isArray(strictResult?.questions)
                            ? strictResult.questions
                            : [];

                    const reconciledQuestions =
                        Array.isArray(reconciliation?.questions)
                            ? reconciliation.questions
                            : [];

                    const questionNumbersFrom = items =>
                        (items || []).map(item =>
                            item?.questionNumber ||
                            item?.question ||
                            item?.no ||
                    ...
                    return {
                        ...
                        rawVisualQuestionNumbers:
                            questionNumbersFrom(rawQuestions),

                        resultQuestionNumbers:
                            questionNumbersFrom(
                                reconciledQuestions
                            ),
                ...
                const processDocxByLocalConvertAndStrictVision = async ({
                    ...
                    if (
                        ...
                        const error = new Error(
                            'DOCX importer 未提供 extractDocxQuestionSkeleton，已停止识别，避免无题号依据的视觉候选进入草稿。'
                        );

                        error.code =
                            'DOCX_SKELETON_API_MISSING';

                        error.stage =
                            'docx-question-skeleton';
                    ...
                    console.log(
                        '[BATCH_DEBUG][docx-question-skeleton-loaded]',
                        {
                            filename:
                                file.filename || '',
                            authoritative:
                    ...
                    const hasConflictingSkeletonEvidence =
                        !questionSkeleton.authoritative &&
                    ...
                    if (
                        hasConflictingSkeletonEvidence
                    ) {
                        const error = new Error(
                        ...
                        error.code =
                            'DOCX_QUESTION_SKELETON_INCOMPLETE';

                        error.stage =
                            'docx-question-skeleton';

                        error.questionSkeleton =
                            questionSkeleton;
                    ...
                    if (
                        ...
                        const error = new Error(
                            'DOCX 题号骨架已标记为 authoritative，但协调器没有应用该骨架。'
                        );

                        error.code =
                            'DOCX_SKELETON_RECONCILE_INVARIANT';

                        error.stage =
                            'docx-question-reconcile';
                    ...
                    console.groupCollapsed(
                        '[BATCH_DEBUG][docx-skeleton-reconcile]'
                    );
                    ...
                    console.log({
                        ...
                        applied:
                            reconciliation.applied,
                        skeletonQuestionNumbers:
                            reconciliation.questionNumbers,
                        rawVisualQuestionNumbers:
                            (strictResult.questions || []).map(
                                item =>
                                    item.questionNumber ||
                                    item.question ||
                                    ''
                    ...
                    const failureSnapshot =
                        buildDocxQuestionFailureSnapshot({
                            file,
                            questionSkeleton,
                            strictResult,
                            reconciliation
                    ...
                    if (
                        ...
                        const error = new Error(
                            'DOCX 显式题号骨架存在，但所有视觉候选均无法与骨架对齐。已停止生成草稿。'
                        );

                        error.code =
                            'DOCX_QUESTION_SKELETON_MISMATCH';
                    ...
                    if (
                        ...
                        error.code =
                            'DOCX_QUESTION_VISUAL_COVERAGE_INCOMPLETE';
                    ...
                    const check = strictResult.check || validateVisualQuestionItems(strictResult.questions, effectiveExpected);
                    ...
                    const questions = (strictResult.questions || []).map(q => ({
                        ...q,
                        sourceFileId: file.id,
                        sourceFileName: file.filename || '',
                    ...
                    const pageImages = (strictResult.pageImages || []).map(img => ({
                        ...img,
                        sourceFileId: file.id,
                        sourceFileName: file.filename || '',
                        convertedPdfFileName: pdfRecord.filename || ''
...
原始文本：${window.Qisi.Utils.cleanRecognizedText(draftRawOptionSource(q)).slice(0, 1800)}`;
                ...
                const compactMergeItemHead = (
                    item = {},
                    valueKey = ''
                ) => ({
                    questionNumber:
                        explicitQuestionNumberOf(item),
                    type:
                        item.type || '',
                    valueHead:
                        window.Qisi.Utils.cleanRecognizedText(
                            valueKey
                                ? (
                                    valueKey === 'answer'
                                        ? getSupportAnswerText(item)
                                        : getSupportSolutionText(item)
                                )
                                : item.stem ||
                                    item.rawBlock ||
                                    ''
                ...
                const normalizeExplicitMergeQuestionNumbers = (
                    values = []
                ) => {
                    const result = [];
                    const seen = new Set();
                ...
                const buildExplicitQuestionMergePlan = ({
                    questionItems = [],
                    answerItems = [],
                    solutionItems = [],
                    ...
                    const diagnostics = {
                        ...
                        duplicateSupportNumbers: [],
                        conflictingSupportNumbers: [],
                        noNumberQuestionCount: 0,
                        noNumberSupportCount: 0,
                        questionHeads: [],
                        answerHeads: [],
                        solutionHeads: []
                    };
                    ...
                    const fail = reason => ({
                        ok: false,
                        code:
                            'EXPLICIT_QUESTION_MERGE_CONTRACT_FAILED',
                        reason,
                        diagnostics
                    });
                    ...
                    if (
                        !authoritativeQuestionContract ||
                        !allowedQuestionNumbers.length
                    ) {
                        return fail(
                            'authoritative-question-contract-empty'
                        );
                    }
                    ...
                    for (const item of questionItems || []) {
                        ...
                        diagnostics.questionHeads.push(
                            compactMergeItemHead(item)
                        );

                        if (!questionNumber) {
                            diagnostics
                                .noNumberQuestionCount += 1;
                            continue;
                        }

                        diagnostics.questionNumbers.push(
                            questionNumber
                        );
                    ...
                    const buildSupportMap = (
                        ...
                        for (const item of items || []) {
                            const questionNumber =
                                explicitQuestionNumberOf(
                                    item
                                );

                            diagnostics[headsKey].push(
                                compactMergeItemHead(
                                    item,
                                    valueKey
                                )
                            );
                            ...
                            if (!questionNumber) {
                                diagnostics
                                    .noNumberSupportCount += 1;
                                continue;
                            }

                            diagnostics[numbersKey].push(
                                questionNumber
                            );
                    ...
                    if ((solutionItems || []).length) {
                        diagnostics.missingSolutionNumbers =
                            allowedQuestionNumbers.filter(
                                questionNumber =>
                                    !solutionMap.has(
                                        questionNumber
                    ...
                    diagnostics.answerNumbers =
                        [...new Set(
                            diagnostics.answerNumbers
                        )];
                    diagnostics.solutionNumbers =
                        [...new Set(
                            diagnostics.solutionNumbers
                        )];
                    diagnostics.unknownQuestionNumbers =
                        [...new Set(
                            diagnostics
                                .unknownQuestionNumbers
                        )];
                    diagnostics.duplicateQuestionNumbers =
                        [...new Set(
                            diagnostics
                                .duplicateQuestionNumbers
                        )];
                    diagnostics.unknownSupportNumbers =
                        [...new Set(
                            diagnostics
                                .unknownSupportNumbers
                        )];
                    diagnostics.duplicateSupportNumbers =
                        [...new Set(
                            diagnostics
                                .duplicateSupportNumbers
                        )];
                    diagnostics.conflictingSupportNumbers =
                        [...new Set(
                            diagnostics
                                .conflictingSupportNumbers
                        )];
                    ...
                    if (
                        ...
                        return fail(
                            'question-items-violate-contract'
                        );
                    }

                    if (
                        diagnostics.noNumberSupportCount ||
                        diagnostics
                            .unknownSupportNumbers
                            .length ||
                        diagnostics
                        ...
                        return fail(
                            'support-items-violate-contract'
                        );
                    }
                ...
                const mergeDraftRecognition = (questionItems, answerItems, solutionItems, fullItems, batch, files, options = {}) => {
                    const now = Date.now();
                    const rawItems = [...questionItems, ...fullItems];
                    const authoritativeQuestionContract =
                        options
                            ?.authoritativeQuestionContract ||
                        null;
                    const explicitMerge =
                    ...
                    let answerMap;
                    let solutionMap;
                    let answerConflicts = new Map();
                    let solutionConflicts = new Map();
                    let explicitMergePlan = null;
                    let matchByOrder = false;

                    if (explicitMerge) {
                        explicitMergePlan =
                            buildExplicitQuestionMergePlan({
                                questionItems:
                                    rawItems,
                                answerItems,
                                solutionItems,
                        ...
                        if (!explicitMergePlan.ok) {
                            ...
                            const error = new Error(
                                `显式题号安全合并失败：${explicitMergePlan.reason}`
                            );
                        ...
                        answerItems = alignSupportItemsSafely(rawItems, answerItems, 'answer');
                        solutionItems = alignSupportItemsSafely(rawItems, solutionItems, 'solution');
                        matchByOrder = shouldMatchByOrder(rawItems, answerItems, solutionItems);
                        items = withMatchKeys(rawItems, matchByOrder);
                        const answerResult = mapItemsByMatchKey(answerItems, matchByOrder, 'answer');
                        const solutionResult = mapItemsByMatchKey(solutionItems, matchByOrder, 'solution');
                        answerMap = answerResult.map;
                        answerConflicts = answerResult.conflicts;
                        solutionMap = solutionResult.map;
                        solutionConflicts = solutionResult.conflicts;
                    }
                    ...
                    const drafts = items.map((item, index) => {
                        ...
                        if (answerConflicts.has(mergeKey)) {
                            ...
                            mergeWarnings.push('answerConflict');
                            duplicateStatus = 'answerConflict';
                        }
                        if (solutionConflicts.has(mergeKey)) {
                            warnings.push('多个解析版本不一致，请确认。');
                        }
                        if (items.filter(other => normalizeQuestionKey(other.question) && normalizeQuestionKey(other.question) === normalizeQuestionKey(item.question)).length > 1) {
                            warnings.push('存在重复题号，请确认是否为不同文件的题号重置。');
                        }
                        ...
                        const solutionIssue = solutionQualityIssue(cleanStem, cleanOptions, cleanSolution);
                        if (solutionIssue) {
                            warnings.push(`匹配到的${solutionIssue}，已自动填入解析区，请人工核对。`);
                            mergeWarnings.push('solutionNeedsReview');
                            if (!explicitMerge && /串题|关键词差异/.test(solutionIssue) && cleanSolution.length < 900) {
                                cleanSolution = '';
                        ...
                        const sourceTraceImageIds = Array.isArray(sourceTrace.imageIds) ? sourceTrace.imageIds : [];
                        const inlineImageIds = inlineImages.map(img => img.id);
                        const mergedImageIds = [...new Set([...sourceTraceImageIds, ...inlineImageIds])];
                        const itemSourcePage = item.sourcePage || item.pageIndex || sourceTrace.sourcePage || sourceTrace.pageIndex || 0;
                        const itemPageIndex = item.pageIndex ?? item.sourcePage ?? sourceTrace.pageIndex ?? 0;
                        const itemSourceFileId = item.sourceFileId || sourceTrace.sourceFileId || '';
                        const itemSourceFileName =
                            item.sourceFileName ||
                        ...
                        return {
                            ...
                            recognizedImages: Array.isArray(item.recognizedImages)
                                ? item.recognizedImages
                                : (Array.isArray(item.images) ? item.images : []),
                            recognizedSolutionImages: Array.isArray(solution?.images) ? solution.images : [],
                            solutionImageRefs: Array.isArray(solution?.imageRefs) ? solution.imageRefs : extractGraphicRefs(cleanSolution),
                            solutionPageImage: solution?.sourcePageImage || '',
                            solutionSourcePage: solution?.sourcePage || 0,
                            sourcePageImage: itemSourcePageImage,
                            sourcePage: itemSourcePage,
                            pageIndex: itemPageIndex,
                            sourceFileId: itemSourceFileId,
                            sourceFileName: itemSourceFileName,
                            rawText: itemRawText,
                            rawBlock: itemRawBlock,
                            pageText: itemPageText,
                            ...
                            imageReviewStatus: inlineImages.length > 0 ? 'confirmed' : 'none',
                            sourceQuestionFileId: itemSourceFileId,
                            sourceAnswerFileId: answer?.sourceFileId || '',
                            sourceSolutionFileId: solution?.sourceFileId || '',
                            answerSource: answer?.sourceFileName || '',
                    ...
                    if (explicitMerge && explicitMergePlan?.ok) {
                        const answerTextLengthsByNumber = {};
                        const solutionTextLengthsByNumber = {};
                        ...
                        console.log(
                            '[BATCH_DEBUG][explicit-question-merge-summary]',
                            {
                                allowedQuestionNumbers:
                                    explicitMergePlan
                                        .allowedQuestionNumbers,
                                ...
                                solutionNumbers:
                                    explicitMergePlan
                                        .diagnostics
                                        .solutionNumbers,
                                answerTextLengthsByNumber,
                                solutionTextLengthsByNumber,
                                mergedAnswerNumbers:
                                    drafts
                                        .filter(draft =>
                                            window.Qisi.Utils.cleanRecognizedText(
                    ...
                    const unmatched = explicitMerge
                        ? []
                        : answerItems
                            .filter((answer, idx) => !usedAnswers.has(matchByOrder ? `#${idx + 1}` : (normalizeQuestionKey(answer.question) || `#${idx + 1}`)))
                            .map(answer => ({ question: answer.question, answer: answer.answer, sourceFile: answer.sourceFileName || '' }));
                    return { drafts, unmatched };
                };

                if (typeof window !== 'undefined') {
                    window.__qisiExplicitQuestionMergeSelfTest = function () {
                        const cases = [];
                        const contractOf = numbers => ({
                            authoritative: true,
                            questionNumbers:
                                numbers.map(String)
                        });
                        const makeQuestions = numbers =>
                            numbers.map(number => ({
                                questionNumber:
                                    String(number),
                                question:
                        ...
                        const expectFailure = (
                            ...
                            expect(
                                failed,
                                'expected explicit merge failure'
                            );
                        });
                        const numbers12 =
                            Array.from(
                        ...
                        runCase(
                            'questions answers solutions 1-12 succeed',
                            () => {
                                const result = merge({
                                    questions:
                                        makeQuestions(numbers12),
                                    answers:
                                        makeAnswers(numbers12),
                                    solutions:
                                        makeSolutions(numbers12),
                                    contract:
                                        contractOf(numbers12)
                                });
                                expect(
                                    result.drafts.length === 12,
                                    'draft count mismatch'
                                );
                                expect(
                                    result.drafts[8].answer ===
                                        'A9',
                                    'answer 9 mismatch'
                                );
                            }
                        ...
                        runCase(
                            'shuffled answers merge by question number',
                            () => {
                                const shuffled =
                                    [...numbers12].reverse();
                                const result = merge({
                                ...
                                expect(
                                    result.drafts[0].answer ===
                                        'A1',
                                    'answer 1 not restored by key'
                                );
                                expect(
                                    result.drafts[11].answer ===
                                        'A12',
                                    'answer 12 not restored by key'
                                );
                            }
                        );

                        expectFailure(
                            'missing answer 9 fails',
                            {
                                questions:
                                    makeQuestions(numbers12),
                                answers:
                        ...
                        expectFailure(
                            'conflicting duplicate answer 5 fails',
                            {
                                questions:
                                    makeQuestions(numbers12),
                                answers: [
                        ...
                        expectFailure(
                            'answer items without question numbers fail',
                            {
                                questions:
                                    makeQuestions(numbers12),
                                answers:
                        ...
                        runCase(
                            'shuffled visual questions merge by question number',
                            () => {
                                ...
                                expect(
                                    result.drafts[0]
                                        .questionNumber ===
                                        '1',
                                    'drafts not ordered by contract'
                                );
                                expect(
                                    result.drafts[11]
                                        .questionNumber ===
                        ...
                        runCase(
                            'existing answer is not overwritten',
                            () => {
                                const result = merge({
                                    questions: [{
                                        question:
                                ...
                                expect(
                                    result.drafts[0].answer ===
                                        'A',
                                    'existing answer was overwritten'
                                );
                            }
                        );

                        runCase(
                            'support raw fields merge by question number',
                            () => {
                                const result = merge({
                                    questions:
                                        makeQuestions([
                                ...
                                expect(
                                    ...
                                    'raw solutions missing after merge'
                                );
                                ...
                                expect(
                                    result.drafts[1]
                                        .solution ===
                                        '解析 2',
                                    'solution 2 not merged by key'
                                );
                            }
                        );

                        runCase(
                            'choice and multiple-choice solutions are always applied from solution map',
                            () => {
                                const result = merge({
                                    questions: [
                                        {
                                    ...
                                    solutions: [
                                        {
                                            question:
                                                '1',
                                            solutionRaw:
                                                '数列等差通项前n项解析 1，按显式题号填入。'
                                        },
                                        {
                                            question:
                                                '2',
                                            solutionRaw:
                                                '数列等差通项前n项解析 2，按显式题号填入。'
                                        },
                                        {
                                            question:
                                                '3',
                                            solutionRaw:
                                                '数列等差通项前n项解析 3，按显式题号填入。'
                                        }
                                    ],
                                    contract:
                                        contractOf([
                                ...
                                expect(
                                    mergedSolutionNumbers
                                        .join(',') ===
                                        '1,2,3',
                                    'merged solution numbers mismatch'
                                );
                                expect(
                                    emptyMergedSolutions
                                        .length === 0,
                                    'empty merged solutions should be empty'
                                );
                            }
                        );
                ...
                const canRenderLatexFragment = (text = '') => {
                    if (!window.katex) return true;
                    const formulas = extractLatexFragmentsForCheck(text);
                ...
                window.__qisiTestBatchDocxGolden = async function (batchId = activeBatchId.value, expectedQuestionCount = 6) {
                ...
                window.__qisiTestBatchVisualGolden = async function (batchId = activeBatchId.value, expectedQuestionCount = 6) {
                    const result = await window.__qisiTestBatchDocxGolden(batchId, expectedQuestionCount);
                ...
                const cleanDocxImporterOptionsForV2 = (options = []) => {
                    const arr = Array.isArray(options) ? options : ['', '', '', ''];
                    return [0, 1, 2, 3].map(idx => cleanDocxImporterTextForV2(arr[idx] || ''));
                };
                ...
                const normalizeDocxMergeQuestionKeyForV2 = (value = '') => {
                    const text = String(value || '')
                ...
                const docxDraftNeedsVisualEnhanceForV2 = (draft) => {
                    ...
                    return hasDocxFormulaPlaceholderForV2(text);
                };

                const docxDraftsNeedVisualEnhanceForV2 = (drafts = []) => {
                    return (drafts || []).some(docxDraftNeedsVisualEnhanceForV2);
                };

                const mergeDocxVisualDraftIntoXmlDraftForV2 = (xmlDraft, visualDraft) => {
                    if (!xmlDraft || !visualDraft) return xmlDraft;
                    ...
                    const beforeOptionPlaceholderCount = countDocxFormulaPlaceholdersForV2(
                    ...
                    const afterOptionPlaceholderCount = countDocxFormulaPlaceholdersForV2(
                        mergedOptions.join('\n')
                    );

                    const originalOptions = Array.isArray(next.options) ? next.options : ['', '', '', ''];

                    const optionsChanged = mergedOptions.some((opt, idx) =>
                        String(opt || '') !== String(originalOptions[idx] || '')
                    );

                    if (optionsChanged || afterOptionPlaceholderCount < beforeOptionPlaceholderCount) {
                        next.options = mergedOptions;
                        warnings.push('已使用 DOCX 转 PDF 后的 OCR 结果修复选项中的公式图片占位。');
                    }

                    next.sourceTrace = {
                        ...(next.sourceTrace || {}),
                        visualEnhancement: 'docx-pdf-ocr-merge',
                        visualQuestionNumber: String(visualDraft.questionNumber || visualDraft.question || visualDraft.order || ''),
                        visualLatexSignals: countLatexSignalsForV2([
                            visualDraft.stem || '',
                            ...((Array.isArray(visualDraft.options) ? visualDraft.options : []))
                        ].join('\n'))
                    };

                    next.warnings = dedupeStringArrayForV2(warnings);
                ...
                const replaceQisiImageTokensForLatex = (source = '', images = [], resolveFilename = null) => {
                    const rows = Array.isArray(images) ? images : [];
                    ...
                    return String(source || '').replace(
                        ...
                        (match, rawId) => {
                            ...
                            if (!image) {
                                console.warn('[LATEX_EXPORT][missing-image]', { id });
                                return `% [缺失图片:${id}]`;
                            }

                            const filename =
                                typeof resolveFilename === 'function'
                                    ? resolveFilename(image)
                                    : (
                                        image.exportFilename ||
                                        image.filename ||
                                        image.name ||
                                        `${image.id || id}.png`
                                    );
                ...
                const normalizeStemImagePlacementTokens = (stem = '') => {
                    let output = String(stem || '');
                    const positionedImageIds = [
                        ...output.matchAll(/\\includegraphics(?:\[[^\]]*\])?\{([^}]+)\}/g)
                    ]
                        .map(match => String(match[1] || '').trim())
                        .filter(Boolean);

                    for (const imageId of positionedImageIds) {
                        const escapedId = escapeImageIdForRegExp(imageId);
                        const bareTokenPattern = new RegExp(
                            `\\n?\\[\\[(?:IMAGE|FORMULA_IMAGE):${escapedId}\\]\\]`,
                            'g'
                        );
                ...
                const normalizeDocxImporterDraftForV2 = (draft, file, batch, order) => {
                    const now = Date.now();
                    const meta = batch.defaultMeta || toRaw(batchDefaultMeta) || {};

                    const rawOptions = Array.isArray(draft.options)
                        ? draft.options
                        : ['', '', '', ''];

                    let options = cleanDocxImporterOptionsForV2(rawOptions);
                    let stem = cleanDocxImporterTextForV2(draft.stem || '');
                    const answer = cleanDocxImporterTextForV2(draft.answer || '');
                    const solution = cleanDocxImporterTextForV2(draft.solution || '');
                    const images = normalizeInlineImageList(draft.images || []);

                    if (options.filter(opt => String(opt || '').trim()).length < 2) {
                        const fallbackSource = [
                            draft.renderableText,
                            draft.rawBlock,
                            draft.sourceTrace?.blockTextHead,
                            draft.stem
                        ].filter(Boolean).join('\n');

                        if (fallbackSource && typeof splitOptionsFromStem === 'function') {
                            const split = splitOptionsFromStem(fallbackSource);
                            if (split && Array.isArray(split.options) && split.options.filter(Boolean).length >= 2) {
                                options = cleanDocxImporterOptionsForV2(split.options);
                                stem = cleanDocxImporterTextForV2(split.stem || stem);
                            }
                        }
                    }
                    ...
                    const normalized = {
                        ...
                        batchId: batch.id,
                        order,
                        questionNumber: String(draft.questionNumber || draft.question || order),
                        type,
                        ...
                        systemKnowledge: draft.systemKnowledge || meta.systemKnowledge || '',
                        personalKnowledge: draft.personalKnowledge || meta.personalKnowledge || '',
                        ...
                        sourceTrace: {
                            ...(draft.sourceTrace || {}),
                            source: 'docx-importer-v2-primary',
                            sourceFileId: file.id || '',
                            sourceFileName: file.filename || '',
                            questionNo: String(draft.questionNumber || draft.question || order)
                        },

                        confidence: images.length ? 0.88 : 0.84,
                        warnings: [
                            ...(Array.isArray(draft.warnings) ? draft.warnings : []),
                            'DOCX 已使用 XML importer 解析，未走 DOCX 转 PDF。'
                        ],
                        ...
                        imageReviewStatus: images.length ? 'need_confirm' : 'none',
                ...
                const buildDocxVisualOcrDraftsForV2 = async (file, batch, engineHelpers = {}) => {
                    console.log('[BATCH_V2][docx-visual-enter]', {
                        filename: file?.filename,
                        hasEngine: Boolean(window.QisiBatchEngineV2?.processBatchV2),
                        href: window.location.href
                    });

                    if (!file || !window.QisiBatchEngineV2?.processBatchV2) return [];

                    let pdfRecord = null;

                    try {
                        pdfRecord = await convertDocxRecordToPdfRecord(file);
                        console.log('[BATCH_V2][docx-visual-convert-ok]', {
                            sourceDocx: file.filename,
                            pdfFilename: pdfRecord.filename,
                            hasUploadPath: Boolean(pdfRecord.uploadPath),
                            uploadPathHead: String(pdfRecord.uploadPath || '').slice(0, 80)
                        });
                    } catch (error) {
                        console.warn('[BATCH_V2][docx-visual-convert-failed]', {
                            filename: file?.filename,
                            message: error?.message || String(error),
                            href: window.location.href
                        }, error);
                    ...
                    try {
                        const visualHelpers = {
                            ...
                            extractPdfLayoutWithPdfJs: null,
                            cropDataUrlByBbox: null,
                            locateQuestionFiguresWithQwen: null
                        };

                        const visualResult = await window.QisiBatchEngineV2.processBatchV2({
                            batch,
                            files: [{
                                ...pdfRecord,
                                id: `${file.id || 'docx'}_visual_pdf`,
                                fileType: 'pdf',
                                role: file.role,
                                roles: getBatchFileRoles(file),
                                filename: `${file.filename || 'docx'}.visual.pdf`
                            }],
                            helpers: visualHelpers
                        });

                        const visualDrafts = visualResult?.drafts || [];

                        console.groupCollapsed('[BATCH_V2][docx-visual-ocr-drafts]');
                        console.log({
                            sourceDocx: file.filename,
                            convertedPdfFilename: pdfRecord.filename,
                            visualDraftCount: visualDrafts.length
                        });
                        console.table(visualDrafts.map(draft => ({
                            q: draft.questionNumber || draft.question || draft.order,
                            stemHead: String(draft.stem || '').slice(0, 120),
                            optionA: draft.options?.[0] || '',
                            optionB: draft.options?.[1] || '',
                            optionC: draft.options?.[2] || '',
                            optionD: draft.options?.[3] || '',
                            latexSignals: countLatexSignalsForV2([
                                draft.stem || '',
                                ...((Array.isArray(draft.options) ? draft.options : []))
                            ].join('\n'))
                        })));
                        ...
                        console.warn('[BATCH_V2][docx-visual-ocr-failed]', {
                            filename: file.filename,
                            message: error?.message || String(error)
                        }, error);
                ...
                const parseDocxQuestionFilesWithImporterForV2 = async (file, batch, engineHelpers = {}) => {
                    ...
                    }
                    {
                        try {
                            ...
                            console.groupCollapsed('[BATCH_V2][docx-visual-trigger-check]');
                            console.log({
                                filename: file.filename,
                                href: window.location.href,
                                isFileProtocol: window.location.protocol === 'file:',
                                normalizedDraftCount: normalizedDrafts.length,
                                needVisualEnhance,
                                drafts: normalizedDrafts.map(draft => ({
                                    q: draft.questionNumber || draft.question || draft.order,
                                    type: draft.type,
                                    stemPlaceholderCount: countDocxFormulaPlaceholdersForV2(draft.stem || ''),
                                    optionPlaceholderCount: countDocxFormulaPlaceholdersForV2((draft.options || []).join('\n')),
                                    stemHead: String(draft.stem || '').slice(0, 160),
                                    optionA: draft.options?.[0] || '',
                                    optionB: draft.options?.[1] || '',
                                    optionC: draft.options?.[2] || '',
                                    optionD: draft.options?.[3] || ''
                            ...
                            if (needVisualEnhance) {
                                warnings.push(`${file.filename || 'DOCX 文件'}：检测到 WMF/EMF/OLE 公式图片占位，已尝试 DOCX→PDF→OCR 视觉增强。`);

                                console.log('[BATCH_V2][docx-visual-start]', {
                                    filename: file.filename,
                                ...
                                if (visualDrafts.length) {
                                    ...
                                    warnings.push(`${file.filename || 'DOCX 文件'}：视觉增强未返回有效结果，暂保留 [公式图片选项待转换] 占位。`);
                                }
                            }

                            drafts.push(...normalizedDrafts);

                            if (Array.isArray(result.draftImages) && result.draftImages.length) {
                                draftImages.push(...result.draftImages);
                            }
                            ...
                            warnings.push(`${file.filename || 'DOCX 文件'}：DOCX XML importer 解析失败：${error?.message || String(error)}`);
                        }
                    }
                ...
                const processDraftImportBatchV2 = async (batchId) => {
                    const now = Date.now();
                    try {
                        console.log('[BATCH_V2][stage]', 'start processDraftImportBatchV2', { batchId });
                        const batch = await db.draftImportBatches.get(batchId);
                        if (!batch) return;
                        activeRecognitionMode = batch.recognitionMode || 'standard';
                        if (!window.QisiBatchEngineV2?.processBatchV2) {
                            throw new Error('批量录题 V2 引擎未加载，请检查 qisi-batch-engine-v2.js 引入顺序。');
                        }
                        ...
                        if (!files.length) throw new Error('当前批量任务没有文件。');
                        ...
                        const docxQuestionFiles = files.filter(file =>
                            file.fileType === 'docx' &&
                            (batchHasQuestionRole(file) || batchIsFullRole(file))
                        );
                        ...
                        const result = {
                            ...
                            draftImages: [
                                ...
                                ...(engineResult.draftImages || [])
                            ],
                            unmatched: engineResult.unmatched || [],
                            warnings: [
                                ...(docxResult.warnings || []),
                        ...
                        let drafts = (result.drafts || []).map((draft, idx) => ({
                            ...draft,
                            batchId,
                            order: idx + 1,
                            updatedAt: Date.now()
                        }));
                        const draftImages = result.draftImages || [];
                        const batchGateResult = batchFinalGateDedupeDrafts(drafts, {
                            stage: 'v2-final-before-bulkPut',
                            batchId,
                            files,
                            draftImages
                        });
