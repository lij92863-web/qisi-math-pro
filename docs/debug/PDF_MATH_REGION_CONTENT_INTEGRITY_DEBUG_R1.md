...
- `verify:docx-stable`: passed（仓库没有 `verify:personal-stable` script，已实际调用并记录 Missing script）。
...
- `verify:personal-stable` 在当前 `package.json` 中不存在；使用仓库现有 `verify:docx-stable` 完成等价稳定链验证。
