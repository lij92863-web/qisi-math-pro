...
### 29.5 收尾：`完整版题目.pdf` 的两页（最后 2 次调用，12/12 用完）

```text
batch B  完整版题目.pdf（question）  2 次调用  → 11 份草稿（题型/选项由视觉文本推断）
  q6 被判 cross-page-visual-block（第 1 页最后一题，检查到它跨页）→ 带原因 withheld，模型原文留证
  正式题库 0 行
```

与同一份卷子的 DOCX 文本（`完整版题目.docx`，确定性抽取）逐题比对，这是付费路线最实在的一次质检：

```text
题序与题面：11 题全部对上（q1-q5、q7-q12）；q6 在两侧都存在，只是扫描侧按跨页规则 withheld
LaTeX 质量：视觉版普遍更好（\mid、\sin、\mathbb{Z}、\tan、\alpha、\cos 都写对了）
真实错误 2 处：q1 把 n 读成 m（同一题在 batch A 里读对过）；q4 "四千年左右" 读成 "四千多年"
题型推断 6 处不同：q7/q8/q9 多选被判成单选，q10/q11/q12 填空被判成解答
一处反向收获：q7 的 $w$ 在 DOCX 链里是 MTEF_UNRESOLVED，视觉链把它读出来了
```

也就是说：**视觉路线的题面还原是可用的，字符级与题型级仍必须人工核对**——每份草稿本来就带
"PDF 视觉转录待人工逐题核对；题号已与文本层验证"的提醒，这 6 处差异就是那句话的实际内容。
两次授权 12 次调用全部用完，正式题库始终 0 行。

## 30. 老师那边为什么还是"0 题"：脚本缓存没有换版本号

2026-09-16 23:54 老师在自己的 app 里跑了 `完整版题目.pdf`，结果仍是
"部分 PDF 内容尚未形成可靠草稿"、0 题 0 已确认 0 已入库。查证过程：

```text
1. 直接向老师正在用的服务取脚本：
   http://127.0.0.1:3000/qisi-pdf-ingestion.js?v=1  → 22224 字节，里面**有**本轮新增的
   repairLatexJsonEscapes，Cache-Control: public, max-age=0
   → 服务端给的是修好后的文件，代码本身在机器上是新的
2. 但 main.html 的脚本 URL 一直写着 ?v=1，本轮改了 5 个文件却一个版本号都没换：
   qisi-utils.js / qisi-docx-ingestion.js / qisi-pdf-ingestion.js / qisi-batch-importer.js / app.js
   → 只要老师那个标签页是在本轮改动之前打开的（那个标签页里 JS 仍在内存里跑），
     不重新加载就还是旧代码；旧代码正是"视觉回复读不出来 → 0 题"的那一版
3. 本轮把 5 个版本号一起换掉，普通刷新就能拿到新脚本，Ctrl+F5 当然也可以
```

给老师的判断方法：批次行右边展开"待核对原页"，看那一行的**原因/服务错误**——
若写 `MALFORMED_MODEL_RESPONSE` 就是标签页还在跑旧脚本（刷新后重跑即可）；
若写 `LOCAL_SERVER_UNREACHABLE` 或其他服务错误，那是模型服务那次调用本身失败了
（本轮已把模型原文留证，重跑一次就能看到它到底回了什么）。

## 31. 真正的原因：混合页把可读文本也一起丢掉了（2026-09-17）

版本号换掉之后，老师在浏览器里重跑**仍然是 0 题**（00:00:11）。这次实测到了环境事实：
这台机器当时连不上模型服务（`dashscope.aliyuncs.com` 连接超时，github 正常）。但真正的问题不在网络，
而在产品逻辑——**模型不可用时，整份文件不该变成 0 题**：

```text
qisi-pdf-inspection.js 的 segment()：page.kind !== 'text' 的页整页跳过
  → mixed 页（公式字形没有 Unicode 映射，但中文正文、题号、答案表都在文本层里读得出来）
    一个字都不进 blocks
  → qisi-pdf-ingestion.js 从 blocks 生成题目 → 一条都没有
  → 视觉调用又失败 → 整批 0 题 + "部分 PDF 内容尚未形成可靠草稿"
```

改法（最小增量，不新建模块）：

```text
1. segment(): 只有 scanned 页跳过；mixed 页照常分段（它仍然进 withheld，仍标"需视觉"）
2. ingestion: 视觉转录到达时，如果同一题已经有"文本版"草稿，用转录版替换它
   （转录带上文本读不出的公式），不新增第二套归属逻辑
3. main.html 两个 PDF 脚本同步换版本号
```

零成本对照（AI 通道被阻断 = 与"模型不可用"同一种情况）：

```text
完整版题目.pdf + 完整版答案.pdf
  before  0 草稿
  after   **11 份草稿**（2 页仍进待核对原页，每题带"PDF 文本提取结果可能不完整，请重点核对"）
```
...
## 33. PDF per-question region：每题区域已经算出来了（2026-09-17）

`qisi-pdf-inspection.js` 的每个 block 现在都带 `regionByPage`：把属于该题的每一行的 bbox 按页求并集，
页眉、页脚、页边和邻题都在框外。`qisi-pdf-ingestion.js` 用它给"需视觉"计划填 `region`——
文本层能证明时给每题区域，证明不了就保持整页（不猜）。

真实材料实测（零成本，AI 阻断）：

```text
完整版题目.pdf p1 q[1..6]   region [90, 183.4, 509.5, 762.1]   ← 不再是整页
完整版题目.pdf p2 q[7..12]  region [90,  75.4, 510.5, 686.5]
完整版答案.pdf 四页          region [0,0,595.3,841.9]（支持文件按设计整页）
草稿数不变：11；verify:safe 1399/1399；tests/pdf-ingestion.test.js 11/11（新增 region 两条）
```

这一条完成的是"程序负责归属"里**归属**的那一半：题号来自文本层，区域来自几何。剩下的一半是
**调用方式**——按区域裁图、一题一请求。那会成倍增加付费调用（一页 6 题 = 6 次而不是 1 次），
属于主人列表第 16 条要求"先提交调用计划再等授权"的范围，因此在拿到新的授权前不动。

## 34. 按区域调用已实现，但模型服务本身连不上（2026-09-17 深夜）

主人授权 6 次之后，调用方式也做完了（`qisi-pdf-ingestion.js`）：一页里每题都有 region 时，
**逐题裁图、逐题请求**，提示词里直接给定题号（"这是第 N 题所在的图片区域…"），模型不再负责定身份；
返回的题号必须等于给定的那一个，否则该题 withheld。没有 region 的页仍走整页老路。测试 12/12，
`verify:safe 1399/1399`。

真实材料上的第一次运行（batch D，授权 6 次）只花掉 **1 次**：

```text
简略版题目（只有一页）.pdf p1 q1 的区域请求 → AI_PROXY_FETCH_FAILED
"DashScope upstream request failed."
→ 首个服务错误终止该文件其余请求（既有规则）→ 只发 1 次，其余 5 次未发、未计费
→ 6 份草稿全部来自文本安全部分（safe partial），正式题库 0 行
```

同时实测本机连通性：`https://dashscope.aliyuncs.com/` 连接超时（github 正常）。也就是说
