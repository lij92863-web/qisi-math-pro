...
> 新增发现（同日已修，见账本 §28）：G7 q19 等六组的最后一题题干里带着答案表的表头行——答案区标题规则
> 只允许 12 字前缀，长标题（20–43 字）匹配不上，切口落在表格的 `答案` 单元格上。现在标题规则只此一处
> （`Qisi.Utils.isAnswerKeyHeadingLine`），六组题干里的表头行全部消失，其余数字一格未变。
...
> **2026-09-16 晚执行结果（第 1–6、8 步已完成，第 7 步等授权）**
>
> | 步 | 状态 | commit |
> | --- | --- | --- |
> | 1 复核 Git + 门禁 | 完成（起点 1385/1385 → 现在 1394/1394） | — |
> | 2 修 §5.1 武汉四调 | 完成：根因是骨架把评分表单元格 `7.0` 当题号（§26），G11 = 14 答 / 19 解 | `f71f1cd` |
> | 3 修 §5.2 佛山一模 | 完成：q3 的 Word tab 选项、q17 的详解插图（§27） | `9c0a8b6` |
> | 4 §5.3 逐页看图 | **部分完成**：G5 p9、G6 p3/p10、G7 p1/p2/p4、G8 p1/p3、G9 p1、G10 p1、G11 p4；十一组答案归属全部对着渲染页文本层核对通过（§8 of DOCX_VISUAL_GROUND_TRUTH） | `d3cd1e3` + 本轮收尾 |
> | 5 统一 matrix + 视觉报告 | 完成：矩阵日志 `astra-docx-final-matrix-heading.log`，报告写入 `DOCX_VISUAL_GROUND_TRUTH_2026_09_16.md` §8 | — |
> | 6 PDF 零成本验收 + 可见性 | 完成：P1/P2/P3 重跑，逐页 withheld 清单 + 审核页可见性实测（`PDF_INSPECTION_STAGE_2026_09_16.md` 末尾） | — |
> | 7 付费视觉方案 | **已提交方案，等明确授权**：`docs/integration/PAID_VISION_PROPOSAL_2026_09_16.md` | — |
> | 8 全量门禁 + Git 审计 + 最终报告 | 完成（verify:safe 1394/1394、verify:batch-safety、账本 §26–§28） | 见最终报告 |
>
> 另外顺手修掉的新缺陷：六组最后一题题干里的答案表表头行（答案区标题规则只允许 12 字前缀，账本 §28）。
> 仍未做：G5/G6/G8/G9/G10/G11 的**逐题**（非逐图）视觉真值，以及 G7 的 p3、p5–p12；这些不标 VISUALLY_VERIFIED。
